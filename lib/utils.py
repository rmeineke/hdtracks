...
def get_html_from_web(page):
    proxies = {"https": "80.187.140.26:8080"}
    page = page + 1
    url = "http://www.hdtracks.com/new?p=" + str(page)
    agent = {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.115 Safari/537.36"
    }
    response = requests.get(url, proxies, headers=agent)
...
def get_artist_info_from_html(html):
    ...
    for item in soup.select(".item"):
        ...
        cursor.execute(
            """SELECT artist, album_name FROM items WHERE artist=? AND album_name=?""",
        ...
        if album:
            ...
            cursor.execute(
                """INSERT INTO items(artist, album_name, sample_rate) VALUES(?,?,?)""",
        ...
        if sample_string == "44.1kHz/16bit":
            ...
            ret_str = (
                ...
                + artist_name
                + "</p>"
                + '<p class="album"><a href="https://www.amazon.com/s?k='
                + artist_name
                + " "
                + album_name
                + '">'
